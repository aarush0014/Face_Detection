...
create_table_sql = f"CREATE TABLE IF NOT EXISTS {table_name} (name TEXT, roll_no INTEGER, time TEXT, date DATE, UNIQUE(roll_no, date))"
...
class Face_Recognizer:
    def __init__(self):
        ...
        self.face_rno_known_list = []#changed
    ...
    def get_face_database(self):
        if os.path.exists("data/features_all.csv"):
            ...
            for i in range(csv_rd.shape[0]):
                ...
                self.face_rno_known_list.append(csv_rd.iloc[i][1])#changed
    ...
    def attendance(self, name, rollno):
        current_date = datetime.datetime.now().strftime('%Y-%m-%d')
        conn = sqlite3.connect("attendance.db")
        cursor = conn.cursor()
        # Check if the name already has an entry for the current date
        cursor.execute("SELECT * FROM attendance WHERE roll_no = ? AND date = ?", (rollno, current_date))
        ...
        if existing_entry:
            ...
            cursor.execute("INSERT INTO attendance (name, roll_no, time, date) VALUES (?, ?, ?, ?)", (name, rollno, current_time, current_date))
            conn.commit()
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                rno =int(self.face_rno_known_list[similar_person_num])#changed

                                print(type(self.face_name_known_list[similar_person_num]))
                                print(type(int(self.face_rno_known_list[similar_person_num])))#changed
                                print(nam)
                                print(rno)#changed
                                self.attendance(nam,rno)#changed
                            else:
